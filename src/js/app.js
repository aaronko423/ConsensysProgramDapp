App = {
  ...
  initWeb3: async function () {
    ...
    var setENS = await web3.eth.ens.setAddress(
      'deployer.eth',
      '0x1a7019793f981897CC51cD8BDF5E9Bd469e43702',
      {
        from: '0x1a7019793f981897CC51cD8BDF5E9Bd469e43702'
      }
    );
    console.log(setENS); */
  ...
  updateUI: function () {
    web3.eth.getAccounts(async function (error, accounts) {
      ...
      welcomeText.innerHTML = `Welcome, ${App.currentAccount}!`;
  ...
  bindEvents: function () {
    ...
    $(document).on('click', '.btn-sell-confirm', App.handleResell);
  ...
  handlePurchase: function (event) {
    ...
    App.contractInstance.transferFrom(App.contractDeployer, App.currentAccount, ticketID, {
      ...
      if (response !== undefined) {
        App.contractInstance.transferEthToCD(ticketID, {
          from: App.currentAccount,
        }).then(function (response){
          if (response !== undefined){
  ...
  handleResell: function (event) {
    ...
    App.contractInstance.getApprovedBuyer(ticketID).then(async function (approvedBuyer){
      if(approvedBuyer !== '0x0000000000000000000000000000000000000000' && approvedBuyer === inputAd.toLowerCase()){
        await App.contractInstance.transferFromSecond(approvedBuyer, ticketID, {from: App.currentAccount});
        await App.contractInstance.transferEthSecond(ticketID, {
          from: App.currentAccount}
        );
  ...
  handleCreateAccount: function (event) {
    ...
    App.contractInstance.createAccount(firstname.toString(), lastname.toString(),
  ...
  handleCreateTicket: function (event) {
    ...
    App.contractInstance.createTicket(name, description, price, {from: App.contractDeployer}).then(async function (response) {
      if (response !== undefined) {
